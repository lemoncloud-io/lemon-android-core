...
class ActivityLauncher<T : Activity>(
     ...
     * @see IntentBuilder
     */
    fun intentBuilder(): IntentBuilder = IntentBuilder.builder(context, activityClass.java)
     ...
     * - 활동을 새로운 테스크로 시작합니다.
     * - 단, 기존 태스크 중에 동일 Affinity를 가진 테스크가 있을 경우 해당 테스크에서 활동이 실행됩니다.
     */
    fun startActivityWithNewTask() {
     ...
     */
    fun startActivityWithNoHistory() {
        context.startActivity(
